...
internal class CallGraphWalker : CSharpSyntaxWalker
{
    ...
    public sealed override void VisitImplicitObjectCreationExpression(ImplicitObjectCreationExpressionSyntax node)
    {
        ...
        {
            if (GetModel(node).GetTypeInfo(node).Type is INamedTypeSymbol typeSymbol)
                VisitAutoGeneratedContructor(typeSymbol);
            return;
        }
    ...
    private void VisitAutoGeneratedContructor(INamedTypeSymbol typeSymbol)
    {
        if (typeSymbol.DeclaringSyntaxReferences.Length == 0)
            return;
        var className = typeSymbol.GetFullMetadataName();
        MethodDeclarationData methodData = new(className, GLSHConstants.Ctor, className, []);

        if (_visitedDeclarations.Add(methodData))
        {
            VisitStructFieldsInitializers(typeSymbol);
            _orderedMethods.Add(methodData);
        }
    }
    ...
    private void VisitContructor(ConstructorDeclarationSyntax ctorDeclaration)
    {
        ...
        string name = GLSHConstants.Ctor;
        ...
        if (_visitedDeclarations.Add(methodData))
        {
            if (GetModel(ctorDeclaration).GetDeclaredSymbol(ctorDeclaration)?.ContainingSymbol is INamedTypeSymbol typeSymbol)
                VisitStructFieldsInitializers(typeSymbol);
    ...
    private void VisitStructFieldsInitializers(INamedTypeSymbol typeSymbol)
    {
        var fields = typeSymbol.GetMembers().Where(m => m.Kind == SymbolKind.Field || m.Kind == SymbolKind.Property);
        foreach (var item in fields)
        {
            ...
            if (syntax is VariableDeclaratorSyntax varDeclarator)
                Visit(varDeclarator.Initializer);
            else if (syntax is PropertyDeclarationSyntax propertyDeclaration)
                Visit(propertyDeclaration.Initializer);
        }
    }
